...
RUN CGO_ENABLED=0 GOFLAGS=-mod=readonly \
    go build -trimpath \
      -ldflags="-s -w \
      -X github.com/kuddy-ai/tabby-sync/internal/version.Version=${VERSION} \
      -X github.com/kuddy-ai/tabby-sync/internal/version.Commit=${COMMIT} \
      -X github.com/kuddy-ai/tabby-sync/internal/version.Date=${BUILD_DATE}" \
      -o /out/tabby-sync ./cmd/tabby-sync
